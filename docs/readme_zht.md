...
fn main() raises:
    ...
    print(a.is_zero())                               # 檢查是否為零: False
    print(Decimal("0").is_zero())                    # 檢查是否為零: True
    
    # === 類型轉換 ===
    print(Float64(a))                                # 轉為浮點數: 123.45
    print(a.to_int())                                # 轉為整數: 123
    print(a.to_str())                                # 轉為字符串: "123.45"
    ...
    print(Decimal("10").log10())                     # 10為底的對數: 1
    print(Decimal("16").log(Decimal("2")))           # 以2為底的對數: 3.9999999999999999999999999999
    ...
    print(Decimal("123.45").is_negative())           # 檢查是否為負: False
    ...
    # === 便捷方法 ===
    print(Decimal("123.400").is_integer())           # 檢查是否為整數: False
...
雖然名稱強調了帶小數部分的十進制數，但 DeciMojo 涵蓋了十進制數學的全部範圍。我們的 `BigInt` 類型雖然只處理整數，但專為十進制數字系統設計，採用以 10^9 為基數的內部表示。這種方法在保持人類可讀的十進制語義的同時提供最佳性能，與專注於二進制的庫形成對比。此外，`BigInt` 作為我們 `BigDecimal` 實現的基礎，使得在整數和小數領域都能進行任意精度的計算。

這個名稱最終強調了我們的使命：為 Mojo 生態系統帶來精確、可靠的十進制計算，滿足浮點表示無法提供的精確算術的基本需求。
...
- **Python decimal**：實現任意精度，可表示具有無限有效位數的數字，但需要動態內存分配和更複雜的算法。
...
[^integer]: BigInt 實現使用基於 10 的表示進行高效存儲和計算，支持對具有無限精度的整數進行操作。它提供了向下整除（向負無窮舍入）和截斷除法（向零舍入）語義，無論操作數符號如何，都能確保除法操作處理具有正確的數學行為。
