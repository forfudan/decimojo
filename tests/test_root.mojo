...
fn test_fractional_inputs() raises:
    ...
    # Test case 4: Decimal input with non-integer result
    var num4 = Decimal("0.5")
    var result4 = root(num4, 2)
    testing.assert_true(
...
fn test_mathematical_identities() raises:
    ...
    testing.assert_true(
        abs(squared_back - x1) < Decimal("0.0000000001"),
    ...
    testing.assert_true(
        abs(root_back - x2) < Decimal("0.0000000001"),
    ...
    testing.assert_true(
        abs(sqrt_product - product_sqrts) < Decimal("0.0000000001"),
    ...
    var x4 = Decimal(5)
    var n = 3  # Cube root
    var power_result = x4 ** (Decimal(1) / Decimal(n))
    var root_result = root(x4, n)
    testing.assert_true(
        abs(power_result - root_result) < Decimal("0.0000000001"),
