"""
Comprehensive tests for the Decimal.from_float() constructor method.
...
fn test_simple_integers() raises:
    ...
    # Test case 1: Zero
    var zero = Decimal.from_float(0.0)
    testing.assert_equal(
        String(zero), "0", "Float 0.0 should convert to Decimal 0"
    )

    # Test case 2: One
    var one = Decimal.from_float(1.0)
    testing.assert_equal(
        String(one), "1", "Float 1.0 should convert to Decimal 1"
    )

    # Test case 3: Ten
    var ten = Decimal.from_float(10.0)
    testing.assert_equal(
        String(ten), "10", "Float 10.0 should convert to Decimal 10"
    )

    # Test case 4: Hundred
    var hundred = Decimal.from_float(100.0)
    testing.assert_equal(
        String(hundred), "100", "Float 100.0 should convert to Decimal 100"
    )

    # Test case 5: Thousand
    var thousand = Decimal.from_float(1000.0)
    testing.assert_equal(
        String(thousand), "1000", "Float 1000.0 should convert to Decimal 1000"
    )
...
fn test_simple_decimals() raises:
    ...
    var half = Decimal.from_float(0.5)
    testing.assert_equal(
        String(half), "0.5", "Float 0.5 should convert to Decimal 0.5"
    )

    # Test case 7: 0.25 (exact representation)
    var quarter = Decimal.from_float(0.25)
    testing.assert_equal(
        String(quarter), "0.25", "Float 0.25 should convert to Decimal 0.25"
    )

    # Test case 8: 1.5 (exact representation)
    var one_half = Decimal.from_float(1.5)
    testing.assert_equal(
        String(one_half), "1.5", "Float 1.5 should convert to Decimal 1.5"
    )

    # Test case 9: 3.14 (check first 3 chars)
    var pi_approx = Decimal.from_float(3.14)
    testing.assert_true(
        String(pi_approx).startswith("3.14"),
        "Float 3.14 should convert to a Decimal starting with 3.14",
    )

    # Test case 10: 2.71828 (check first 6 chars)
    var e_approx = Decimal.from_float(2.71828)
    testing.assert_true(
        String(e_approx).startswith("2.7182"),
        "Float 2.71828 should convert to a Decimal starting with 2.7182",
    )
...
fn test_negative_numbers() raises:
    ...
    # Test case 11: -1.0
    var neg_one = Decimal.from_float(-1.0)
    testing.assert_equal(
        String(neg_one), "-1", "Float -1.0 should convert to Decimal -1"
    )

    # Test case 12: -0.5
    var neg_half = Decimal.from_float(-0.5)
    testing.assert_equal(
        String(neg_half), "-0.5", "Float -0.5 should convert to Decimal -0.5"
    )

    # Test case 13: -123.456 (check first 7 chars)
    var neg_decimal = Decimal.from_float(-123.456)
    testing.assert_true(
        String(neg_decimal).startswith("-123.45"),
        "Float -123.456 should convert to a Decimal starting with -123.45",
    )

    # Test case 14: -0.0 (negative zero)
    var neg_zero = Decimal.from_float(-0.0)
    testing.assert_equal(
        String(neg_zero), "0", "Float -0.0 should convert to Decimal 0"
    )

    # Test case 15: -999.999 (check first 7 chars)
    var neg_nines = Decimal.from_float(-999.999)
    testing.assert_true(
        String(neg_nines).startswith("-999.99"),
        "Float -999.999 should convert to a Decimal starting with -999.99",
    )
...
fn test_very_large_numbers() raises:
    ...
    # Test case 16: 1e10
    var ten_billion = Decimal.from_float(1e10)
    testing.assert_equal(
        String(ten_billion),
        "10000000000",
        "Float 1e10 should convert to Decimal 10000000000",
    )

    # Test case 17: 1e15
    var quadrillion = Decimal.from_float(1e15)
    testing.assert_equal(
        String(quadrillion),
        "1000000000000000",
        "Float 1e15 should convert to Decimal 1000000000000000",
    )

    # Test case 18: Max safe integer in JavaScript (2^53 - 1)
    var max_safe_int = Decimal.from_float(9007199254740991.0)
    testing.assert_equal(
        String(max_safe_int),
        "9007199254740991",
        "Float 2^53-1 should convert to exact Decimal 9007199254740991",
    )

    # Test case 19: 1e20
    var hundred_quintillion = Decimal.from_float(1e20)
    testing.assert_equal(
        String(hundred_quintillion),
        "100000000000000000000",
        "Float 1e20 should convert to Decimal 100000000000000000000",
    )

    # Test case 20: Large number with limited precision
    var large_number = Decimal.from_float(1.23456789e15)
    testing.assert_true(
...
fn test_very_small_numbers() raises:
    ...
    # Test case 21: 1e-10
    var tiny = Decimal.from_float(1e-10)
    testing.assert_true(
        String(tiny).startswith("0.00000000"),
        "Float 1e-10 should convert to a Decimal with appropriate zeros",
    )

    # Test case 22: 1e-15
    var tinier = Decimal.from_float(1e-15)
    testing.assert_true(
        String(tinier).startswith("0.000000000000001"),
        "Float 1e-15 should convert to a Decimal with appropriate zeros",
    )

    # Test case 23: Small number with precision
    var small_with_precision = Decimal.from_float(1.234e-10)
    ...
    var very_small = Decimal.from_float(1e-20)
    testing.assert_true(
        String(very_small).startswith("0.00000000000000000001"),
        "Very small float should convert to appropriate Decimal",
    )

    # Test case 25: Denormalized float
    var denorm = Decimal.from_float(1e-310)
    testing.assert_true(
        String(denorm).startswith("0."),
        "Denormalized float should convert to small Decimal",
    )
...
fn test_binary_to_decimal_conversion() raises:
    ...
    var point_one = Decimal.from_float(0.1)
    testing.assert_true(
        String(point_one).startswith("0.1"),
        "Float 0.1 should convert to a Decimal starting with 0.1",
    )

    # Test case 27: 0.2 (known inexact in binary)
    var point_two = Decimal.from_float(0.2)
    testing.assert_true(
        String(point_two).startswith("0.2"),
        "Float 0.2 should convert to a Decimal starting with 0.2",
    )

    # Test case 28: 0.3 (known inexact in binary)
    var point_three = Decimal.from_float(0.3)
    testing.assert_true(
        String(point_three).startswith("0.3"),
        "Float 0.3 should convert to a Decimal that starts with 0.3",
    )

    # Test case 29: 0.1 + 0.2 (famously != 0.3 in binary)
    var point_one_plus_two = Decimal.from_float(0.1 + 0.2)
    testing.assert_true(
        String(point_one_plus_two).startswith("0.3"),
        "Float 0.1+0.2 should convert to a Decimal starting with 0.3",
    )

    # Test case 30: Repeating binary fraction
    var repeating = Decimal.from_float(0.1)
    testing.assert_true(
...
fn test_rounding_behavior() raises:
    """Test rounding behavior during float to Decimal conversion."""
    print("Testing rounding behavior in float to Decimal conversion...")

    # Test case 31: Pi with limited precision
    var pi = Decimal.from_float(3.141592653589793)
    testing.assert_true(
        String(pi).startswith("3.14159265358979"),
        "Float Pi should maintain appropriate precision in Decimal",
    )

    # Test case 32: 1/3 (repeating decimal in base 10)
    var one_third = Decimal.from_float(1.0 / 3.0)
    testing.assert_true(
        String(one_third).startswith("0.33333333"),
        "Float 1/3 should maintain appropriate precision in Decimal",
    )

    # Test case 33: 2/3 (repeating decimal in base 10)
    var two_thirds = Decimal.from_float(2.0 / 3.0)
    testing.assert_true(
        String(two_thirds).startswith("0.66666666"),
        "Float 2/3 should maintain appropriate precision in Decimal",
    )

    # Test case 34: Round trip conversion
    var x = 123.456
    var decimal_x = Decimal.from_float(x)
    testing.assert_true(
        String(decimal_x).startswith("123.456"),
        "Float-to-Decimal conversion should preserve input precision",
    )

    # Test case 35: Number at float precision boundary
    var precision_boundary = Decimal.from_float(9.9999999999999999)
    testing.assert_true(
...
fn test_special_values() raises:
    ...
    var epsilon = Decimal.from_float(2.220446049250313e-16)
    testing.assert_true(
...
fn test_scientific_notation() raises:
    ...
    var sci1 = Decimal.from_float(1.23e5)
    testing.assert_equal(
        String(sci1),
        "123000",
    ...
    var sci2 = Decimal.from_float(4.56e-3)
    testing.assert_true(
    ...
    var sci3 = Decimal.from_float(1.0e20)
    testing.assert_equal(
        String(sci3),
        "100000000000000000000",
    ...
    var sci4 = Decimal.from_float(1.0e-10)
    testing.assert_true(
    ...
    var sci5 = Decimal.from_float(5e20)
    testing.assert_true(
...
fn test_boundary_cases() raises:
    ...
    var beyond_safe = Decimal.from_float(9007199254740994.0)
    testing.assert_true(
    ...
    var trailing_zeros = Decimal.from_float(123.000000)
    testing.assert_true(
    ...
    var fraction = Decimal.from_float(0.125)  # 1/8, exact in binary
    testing.assert_equal(
        String(fraction),
        "0.125",
        (
...
fn main() raises:
    print("=========================================")
    print("Running 50 tests for Decimal.from_float()")
    print("=========================================")
    ...
    print("All 50 Decimal.from_float() tests passed!")
