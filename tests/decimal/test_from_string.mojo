"""
Comprehensive tests for the Decimal.from_string() constructor method.
...
fn test_basic_integers() raises:
    ...
    # Test case 1: Zero
    var zero = Decimal.from_string("0")
    testing.assert_equal(
        String(zero), "0", "String '0' should convert to Decimal 0"
    )

    # Test case 2: One
    var one = Decimal.from_string("1")
    testing.assert_equal(
        String(one), "1", "String '1' should convert to Decimal 1"
    )

    # Test case 3: Simple integer
    var simple_int = Decimal.from_string("123")
    testing.assert_equal(
        String(simple_int), "123", "String '123' should convert correctly"
    )

    # Test case 4: Large integer
    var large_int = Decimal.from_string("123456789")
    testing.assert_equal(String(large_int), "123456789")

    # Test case 5: Integer with internal spaces (should fail)
    var exception_caught = False
    try:
        var _invalid = Decimal.from_string("1 234")
        testing.assert_equal(
...
fn test_negative_numbers() raises:
    ...
    var neg_small = Decimal.from_string("-0.001")
...
fn test_formatting_variants() raises:
    ...
    var leading_zeros_int = Decimal.from_string("00123")
    testing.assert_equal(String(leading_zeros_int), "123")

    # Test case 27: Trailing zeros after decimal point
    var trailing_zeros = Decimal.from_string("123.4500")
    testing.assert_equal(String(trailing_zeros), "123.4500")

    # Test case 28: Both leading and trailing zeros
    var both_zeros = Decimal.from_string("00123.4500")
    testing.assert_equal(String(both_zeros), "123.4500")

    # Test case 29: Decimal point with no following digits
    var decimal_no_digits = Decimal.from_string("123.")
    testing.assert_equal(
        String(decimal_no_digits),
        "123",
        "Decimal point with no digits should be ignored",
    )

    # Test case 30: Decimal point with no preceding digits
    var decimal_no_preceding = Decimal.from_string(".123")
    testing.assert_equal(
        String(decimal_no_preceding),
        "0.123",
        "Decimal point with no preceding digits should add leading 0",
    )
...
fn test_invalid_inputs() raises:
    ...
    var exception_caught = False
    try:
        var _empty_string = Decimal.from_string("")
    ...
    try:
        var _non_numeric = Decimal.from_string("abc")
        testing.assert_equal(
    ...
    exception_caught = False
    try:
        var _multiple_points = Decimal.from_string("1.2.3")
        testing.assert_equal(
    ...
    exception_caught = False
    try:
        var _invalid_exp = Decimal.from_string("1.23e")
        testing.assert_equal(
    ...
    exception_caught = False
    try:
        var _mixed = Decimal.from_string("123a456")
        testing.assert_equal(
...
fn test_boundary_cases() raises:
    ...
    var max_value = Decimal.from_string(max_value_str)
...
fn test_special_cases() raises:
    ...
    var long_decimal = Decimal.from_string(
...
fn main() raises:
    print("=========================================")
    print("Running 50 tests for Decimal.from_string()")
    print("=========================================")
    ...
    print("All 50 Decimal.from_string() tests passed!")
