...
struct BigBinaryUInt:
    ...
    """The base used for the BigBinaryUInt representation."""
    alias BASE_MAX = (1 << 30) - 1  # 2^30 - 1 = 1073741823
    """The maximum value of a single word in the BigBinaryUInt representation."""
    alias BASE_HALF = 1 << 29  # 2^29 = 536870912
    """Half of the base used for the BigBinaryUInt representation."""
    ...
    fn __init__(out self, owned *words: UInt32):
        """Initializes a BigBinaryUInt from raw words without validating the words.
    ...
    fn from_list(owned words: List[UInt32]) raises -> Self:
        """Initializes a BigBinaryUInt from a list of UInt32 words safely.
        If the list is empty, the BigBinaryUInt is initialized with value 0.
        ...
        Returns:
            The BigBinaryUInt representation of the list of UInt32 words.
        """
        ...
        for word in words:
            if word >= Self.BASE:
                raise Error(
                    "Error in `BigBinaryUInt.from_list()`: Word value exceeds"
