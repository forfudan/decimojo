...
"""
Shared benchmark utilities for DeciMojo benchmarks.

Provides common infrastructure for:
- Benchmark case loading from TOML files
- Log file management
- Formatted output (console + log)
...
Pattern expansion in operand values:
    ...
    Examples:
        ...
        "-{9,50}"      → "-999...9"
        "{9,999}1"     → "999...91"
        "{123456789,5}" → "123456789123456789123456789123456789123456789"
"""
...
fn load_bench_cases(toml_path: String) raises -> List[BenchCase]:
    ...
    Uses Python's tomllib (Python 3.11+) to parse the TOML file at runtime.
    Operand values are expanded using the {C,N} pattern syntax.
    ...
    var f = builtins.open(toml_path, "rb")
    var data = tomllib.load(f)
    f.close()

    var cases = List[BenchCase]()
    var toml_cases = data["cases"]

    for i in range(len(toml_cases)):
        var tc = toml_cases[i]
        var name = String(tc["name"])
        var a = expand_value(String(tc["a"]))

        # b is optional for unary operations
        var b = String("")
        try:
            b = expand_value(String(tc["b"]))
        except:
            pass
...
fn load_bench_iterations(toml_path: String) raises -> Int:
    ...
    try:
        ...
        var py_val = py_builtins.int(data["config"]["iterations"])
        return atol(String(py_val))
    except:
        return 1000
